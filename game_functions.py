...
def check_events(game_set, screen, p1, p2, bullets1, bullets2, play_button, score, sound):
    """Check keyboard/mouse events, delegate them to appropriate event functions"""
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            sys.exit()

        elif event.type == pygame.KEYDOWN:
            check_keydown_events(event, game_set, screen, p1, p2, bullets1, bullets2, sound)
            ...
            check_play_button(game_set, screen, p1, p2, bullets1, bullets2, play_button, mouse_x, mouse_y, score, sound)


def check_keydown_events(event, game_set, screen, p1, p2, bullets1, bullets2, sound):
    ...
    if event.key == pygame.K_w:
        ...
        fire_bullet1(game_set, screen, p1, bullets1, sound)
    elif event.key == pygame.K_RSHIFT:
        fire_bullet2(game_set, screen, p2, bullets2, sound)
...
def check_play_button(game_set, screen, p1, p2, bullets1, bullets2, play_button, mouse_x, mouse_y, score, sound):
    """Start a new game when the player clicks Play."""
    button_clicked = play_button.rect.collidepoint(mouse_x, mouse_y)
    if button_clicked and not game_set.game_active:
        sound.confirm.play()  # pressed button sound effect
        ...
        score.prep_players()
        score.prep_lives()
        sound.play(sound.battle_music, -1)
...
def player_obs_col(game_set, screen, p1, p2, obstacles, bullets1, bullets2, score, sound):
...
def reset_stage(game_set, screen, p1, p2, bullets1, bullets2, sound):
    """Show winning screen if any player have no lives left, else just reset"""
    if game_set.p1_lives == 0:
        victory_screen('images/bg_dirt_p2win.bmp', screen, game_set, sound)

    elif game_set.p2_lives == 0:
        victory_screen('images/bg_dirt_p1win.bmp', screen, game_set, sound)
...
def victory_screen(image, screen, game_set, sound):
    ...
    sound.play(sound.victory_music, 0)
    sleep(4)
    game_set.game_active = False
    sound.play(sound.title_music, -1)
...
def fire_bullet1(game_set, screen, player, bullets, sound):
...
def fire_bullet2(game_set, screen, player, bullets, sound):
    # checking if bullets can be fired before doing so
...
def update_bullets(game_set, screen, p1, p2, obstacles, bullets1, bullets2, score, sound):
    ...
    player_obs_col(game_set, screen, p1, p2, obstacles, bullets1, bullets2, score, sound)
